...
public class DebugCommand implements CommandExecutor {
    static ActionItem<Player> item1 = new ActionItem<>(new ItemStack(Material.BARRIER), event -> event.getPlayer().closeInventory(), "Barrier");
    static ActionItem<Player> item2 = new ActionItem<>(new ItemStack(Material.BED), event -> event.getPlayer().getWorld().setTime(20000), "Bed");
    static ActionItem<Player> item3 = new ActionItem<>(new ItemStack(Material.EGG), event -> event.getPlayer().sendMessage("huevo"), "Huevo");
    ...
    static ChestMenu chestMenu1 = new ChestMenu("chestMenu1", contents, true, InventoryMenu.SaveOption.NONE, false, 2, 3);
    static ChestMenu chestMenu2 = new ChestMenu("chestMenu2", new ItemStack[]{new ItemStack(Material.POISONOUS_POTATO), new ItemStack(Material.BAKED_POTATO)}, true, InventoryMenu.SaveOption.GLOBAL, false, 1);
    static ActionItem<Player> playerSkull = new ActionItem<>(pl -> Utils.setName(new ItemStack(Material.SKULL_ITEM), pl != null ? pl.getName() : ""), null, "playerSkull");
    ...
    static MenuItem<ChestMenu, Player> menu1 = new MenuItem<>(new ItemStack(Material.CHEST), chestMenu1, "chestMenu1");
    static MenuItem<ChestMenu, Player> menu2 = new MenuItem<>(new ItemStack(Material.ENDER_CHEST), chestMenu2, "chestMenu2");
    //static MenuItem<ChestMenu> menu3 = new MenuItem<>(new ItemStack(Material.SKULL_ITEM), chestMenu3, "chestMenu3");
    static MenuItem<AnvilMenu, Player> menu4 = new MenuItem<>(new ItemStack(Material.ANVIL), anvilMenu1, "anvilMenu1");
    static MenuItem<InventoryMenu, Player> menuTest = new MenuItem<>(new ItemStack(Material.POTION), new InventoryMenu("brewing", true, InventoryMenu.SaveOption.NONE, InventoryType.BREWING, new ItemStack[4]), "menuTest");
    ...
    */static BookMenu.Field contentsField1 = new BookMenu.Field("example", true, true);
    static BookMenu.InventoryField contentsField2 = new BookMenu.InventoryField(new ChestMenu("InvMenuInBookMenu", new ItemStack[]{new ItemStack(Material.CAKE), null}, true, InventoryMenu.SaveOption.GLOBAL, false));
    static {
        ((ChestMenu) contentsField2.getInvMenu()).setFillWithBarriers(true);
    }
    static BookMenu<?> bookMenu1 = new BookMenu<>(contentsField1);
    static MenuItem<BookMenu<?>, Player> menu6 = new MenuItem<>(new ItemStack(Material.BOOK), bookMenu1, "bookMenu1");
    ...
    static MenuItem<BookMenu<?>, Player> menu7 = new MenuItem<>(new ItemStack(Material.BOOK), bookMenuFromConfig, "bookMenu2");
    static ListItem<Player> listItem = new ListItem<>(arg -> Utils.setName(new ItemStack(Material.PAPER), "list: " + arg.getDisplayName()),
            Arrays.asList("a", "b", "c"), pl -> pl.getFoodLevel() % 3, true, "ListItem");
    static MenuItem<ChestMenu, Player> menu8 = new MenuItem<>(new ItemStack(Material.COMMAND_MINECART), new ChestMenu("menu8",
            new ItemStack[27], true, InventoryMenu.SaveOption.INDIVIDUAL, false), "menu8");
    static MenuItem<ChestMenu, Player> menu9 = new MenuItem<>(new ItemStack(Material.COMMAND_MINECART), new ChestMenu("menu9",
            new ItemStack[1], true, InventoryMenu.SaveOption.INDIVIDUAL, false), "menu9");
    static {
        menu9.getMenu().setFillWithBarriers(true);
    }
    static MenuItem<ChestMenu, Player> menu10 = new MenuItem<>(new ItemStack(Material.CHEST), new ChestMenu("menu10",
            player -> {
        ItemStack[] toret = new ItemStack[100];
                for (int i = 0; i < toret.length; i++) {
                    toret[i] = new ItemStack(Material.values()[(int) Math.floor(Math.random() * Material.values().length) % Material.values().length]);
                }
                return toret;
            }, true, InventoryMenu.SaveOption.GLOBAL, true), "menu10");
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("AnotherGUIPlugin.menusDebug"))
            return false;
        ...
        //        menu8.toItemStack(), menu9.toItemStack(), menu10.toItemStack());
        player.getInventory().addItem(Skulls.getPlayerHead(player.getName()));
        for (String arg : args)
            player.getInventory().addItem(Skulls.getPlayerHead(arg));
